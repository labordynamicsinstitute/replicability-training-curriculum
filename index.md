...
The given name should be `template-config.do`. In order to use it, rename it to `config.do` and move it to the openICPSR folder (e.g. , 140XXX).

### Include config.do
- Add the following line at the beginning of each code:
    ```
    include config.do
    ```
    and add the following at the end of each code:
    ```
    log close
    ```
- If there is a master do file, you can choose to put the above line at the beginning and the end of the `master.do` once.

- There will be cases where authors create their own log files. In that case, one thing you can do is to comment out line 27. 
